...
class LiveRoomController extends MainController
{
    ...
    public function show($id, Content $content)
    {
        ...
        // Paginate at the query level and push the mic-position ordering into
        // SQL (LEFT JOIN room_microphones, NULLs last) so we never load the
        // whole visitor set into PHP. mic_position/kick_info are computed only
        // for the current page's rows below.
        $visitors = $room->roomVisitors()
            ->with('user.profile')
            ->leftJoin('room_microphones', function ($join) use ($room) {
                $join->on('room_microphones.user_id', '=', 'room_visitors.user_id')
                    ->where('room_microphones.room_id', '=', $room->id)
                    ->where('room_microphones.user_id', '>', 0);
            })
            ->select('room_visitors.*')
            ->orderByRaw('room_microphones.position IS NULL, room_microphones.position ASC')
            ->paginate(15);
    ...
    protected function setupBaseModel(Grid $grid, $user): void
    {
        ...
        $grid->model()
            ->where('is_live', 1)
            ->select("id", 'uid', 'microphone', 'pin', 'max_admin', 'pin', 'is_top', 'top_room', "room_name", "room_cover", "room_admin", 'session', \DB::raw("
                CASE room_status
                    WHEN 1 THEN 100
                    WHEN 2 THEN 10
                    ELSE 80
    ...
    {
        if (!Admin::user()->can('*')) Permission::check('edit-rooms');
        ...
        try {
            ...
            Log::error('Error updating pin status', ['room_id' => $id, 'exception' => $e]);
            return response()->json([
                'success' => false,
                'message' => __('Error updating pin status')
            ]);
        }
    }
