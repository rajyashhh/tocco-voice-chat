...
class SVGAAssetCacheManager {
  static final SVGAAssetCacheManager _instance =
      SVGAAssetCacheManager._internal();
  factory SVGAAssetCacheManager() => _instance;
  SVGAAssetCacheManager._internal() {
    _initAsync();
  }
  Dio get _dio => di<Dio>(instanceName: 'downloadDio');
  static const String _hiveBoxName = 'svga_cache_box';
  ...
  void _initAsync() {
    _init().catchError((e) {
      Methods.printLog('[SVGACache] init error: $e');
    });
  }
  ...
  Future<void> initHive() async {
    ...
    try {
      ...
      Methods.printLog('[SVGACache] Hive init error: $e');
      rethrow;
    } finally {
  ...
  Future<void> clearAllCache() async {
    try {
      ...
      Methods.printLog('[SVGACache] clearAllCache error: $e');
    }
  }
    ...
    try {
      ...
      for (final key in keys) {
        ...
        if (now.difference(stat.modified) > _maxAge) {
          ...
          fileMeta.add(_SvgaFileMeta(key, stat.size, stat.modified));
        }
      }
      ...
      Methods.printLog('[SVGACache] eviction error: $e');
    }
  }
}
...
class _SvgaFileMeta {
  ...
  const _SvgaFileMeta(this.key, this.size, this.modified);
}
