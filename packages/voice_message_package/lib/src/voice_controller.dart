...
class VoiceController extends MyTicker {
  ...
  /// Local fork fix: dispose was not idempotent — a second call reached
  /// AnimationController.dispose() again, whose internal `_ticker!` is null
  /// after the first dispose (Null check operator crash, Crashlytics
  /// voice_controller.dart:187). Streams are also cancelled before the
  /// player is disposed so no position callback runs against a disposed
  /// AnimationController.
