...
class HomeController extends  MainController
{
    ...
    /**
     * Country scope for the game endpoints: this super-admin's own country,
     * optionally narrowed by an explicit ?country_id= override (honored only when
     * it matches the base scope, so isolation is never widened). Always returns a
     * non-empty array — game stats are never global here.
     */
    private function gameCountryScope(Request $request): array
    {
        $base = (int) $this->countryId();
    ...
    public function financeCards(Request $request)
    {
        ...
        $result = UserSallary::whereHas('user', fn($q) => $q->where('country_id', $countryID))
    ...
    public function financeChartIndex(Request $request)
    {
        ...
        $charges = Charge::whereHas('user', fn($q) => $q->where('country_id', $countryID))
