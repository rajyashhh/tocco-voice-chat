...
class UserController extends MainController
{
    ...
    {
        User::whereIn('country_id', Common::areaCountries())->findOrFail($id);
    ...
    public function show($id, Content $content,)
    {
        ...
        $user = User::with('profile')->whereIn('country_id', Common::areaCountries())->findOrFail($id);
        $type = request('type') ?? 4;
   ...
   Admin::style(UserService::adminUserCardStyles() . gridStyles());
    ...
    {
        $user = User::with(['sameDeviceUsers.profile'])->whereIn('country_id', Common::areaCountries())->findOrFail($id);
